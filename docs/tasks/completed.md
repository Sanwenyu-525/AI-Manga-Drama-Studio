...
|---|---|---|---|---|
| P1-E4-T03 | 建立关联日志与深度健康检查 | 2026-09-06 | （见 git log） | logging 四字段 contextvar（correlation/project/run/generation）+ HTTP/Agent/Worker 绑定 + gateway_status + /health 四组件（database/worker/event_bus/providers）与 degraded/unhealthy 语义；test_logging_health.py 新增 10 项；契约 §140 同步 |
| P1-E6-T02 | 修正文档事实源与移除误导性遗留入口 | 2026-09-06 | （见 git log） | AGENTS 技术栈→DB-poll；design-qa 两处 Stage D 改历史口径；README 计数 682/299 + 能力矩阵（默认/真实验证）；current-state §8 标记关闭；遗留 DirectorPanel 确认已删（仅 live 版）；test_docs_factsource.py 新增 4 项防漂移 |
| P2-E1-T02 | 支持长文本分块、进度与明确角色提取 | 2026-09-06 | （见 git log） | 分块分析（6000字/块≤4块，超限422+分集提示，失败零落库重试一致）+ preview/latest 范围成本字段 + 角色候选审阅与决策接口（create/merge/skip逐项，冲突/跨项目守卫）+ 前端范围提示与候选面板；迁移 b2c3d4e5f6a7（快照候选列）；契约 §14.2；pytest 8 + vitest 3 |
| P2-E2-T01 | 补齐核心实体归档、恢复与删除语义 | 2026-09-06 | （见 git log） | restore×5（幂等/404/父级409/编号409）+ trash + 项目列表include_deleted + 项目级联扩至地点/服装/文档 + episode部分唯一索引迁移 d4e5f6a7b8c9 + undo-delete经restore补偿 + 前端回收站/已删项目/事件映射/文案；契约 §12.3/§56-57/§143 + database §10.5/§41.1；pytest 9+2更新 + vitest 3 |
| P2-E2-T02 | 建立项目作用域 Asset Library | 2026-09-06 | （见 git log） | cursor分页（offset保留兼容，互斥422）+ source/shot/scene/created范围筛选（跨项目422/不存在404）+ 导入校验（伪装MIME/空文件/source_name清洗/可选shot_id归属校验，meta关联可筛选）+ 详情追溯（integrity全量重算/version active+MASTER/shot→scene→episode，generation走provenance）+ 归档/恢复/真删（active·MASTER·clip·服装引用409+清单，未归档/缺confirm 422，事件asset.archived/restored/deleted）+ 前端useInfiniteQuery分页器/列表Error+重试/Inspector完整性行；零Schema变更；契约 P6-B/P2-E2-T02段+§143 + database §41.1 + frontend-ux §26；pytest 31 + vitest 4（assetBrowser）|
