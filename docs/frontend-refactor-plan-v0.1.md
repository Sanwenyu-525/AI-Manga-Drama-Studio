...
> 状态：**R0 / R1 Done（2026-08-23）**；样式基线恢复（`2cfcb3d`）、Episode-aware canonical 路由与状态单一事实源（`2a6f143`）已落地；R0/R1 收尾（a11y + 响应式布局）随本计划提交。**R2 Done（2026-08-23）**：响应式三档（1440/1220/1100）实机验证通过、`.episode-panel` 移除固定 `min-width:920px`（改 `0`）、仅 Timeline 可横向滚动、顶部状态按媒体查询分级隐藏；Shell 在 1440×900、1220×768、1100×768、1920×1080 均无页面级横向溢出。R3 / R4 未开始。  
> 审计日期：2026-08-21  
...
> **状态：Done（2026-08-23）** —— 六项需求已落实并经浏览器实测（1440×900 / 1220×768 / 1100×768 / 1920×1080）通过。`.episode-panel` 固定 `min-width:920px` 已移除（改 `0`），`app-shell` 5 列 Grid 在媒体查询下按需求收紧列宽并保持一致 DOM 结构；顶部导航/状态按 1220 / 1100 断点分级隐藏；全场仅 Timeline 允许横向滚动，Shell 在 4 档视口下均无页面级横向溢出。
