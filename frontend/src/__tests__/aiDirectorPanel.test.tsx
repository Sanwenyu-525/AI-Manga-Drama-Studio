...
function wrapper() {
  ...
  return { qc, W: ({ children }: { children: React.ReactNode }) => <QueryClientProvider client={qc}>{children}</QueryClientProvider> };
}
...
describe("AIDirectorPanel (WAITING_HUMAN)", () => {
  it("shows the 等待审批 badge and the approval hint", async () => {
    ...
    const get = vi.fn().mockImplementation((p: string) => {
      ...
      return Promise.resolve({ id: "run_1", status: "WAITING_HUMAN", project_id: "p1", plan: null, approval: null, change_set_id: null, result: null, pending_proposals: [], created_at: "", updated_at: "" });
    });
  ...
  it("renders the 待审批 Proposal review section when a run is pending", async () => {
    ...
    const get = vi.fn().mockImplementation((p: string) => {
      if (p.endsWith("/proposals")) return Promise.resolve([{ id: "prop_1", tool: "update_shot", target_type: "shot", target_id: "shot_xyz", base_revision: 2, changes: { image_prompt: { from: "a", to: "b" } }, status: "pending" }]);
      return Promise.resolve({ id: "run_2", status: "WAITING_HUMAN", project_id: "p1", plan: null, approval: null, change_set_id: null, result: null, pending_proposals: [], created_at: "", updated_at: "" });
    });
    vi.spyOn(client.api, "get").mockImplementation(get);
    const { W } = wrapper();
    render(<MemoryRouter initialEntries={["/projects/p1/episodes/e1/script"]}><AIDirectorPanel /></MemoryRouter>, { wrapper: W });
