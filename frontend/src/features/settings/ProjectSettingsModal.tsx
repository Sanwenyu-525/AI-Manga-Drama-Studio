...
export function ProjectSettingsModal({ projectId, open, onClose }: ProjectSettingsModalProps) {
  ...
  const save = useMutation({
    mutationFn: (update: ProjectSettingUpdate) =>
      api.put<ProjectSettings>("/projects/" + projectId + "/settings", update),
    onSuccess: (updated) => {
  ...
  const dirty = form ? JSON.stringify(form) !== JSON.stringify(settings ? toForm(settings) : null) : false;
      ...
      <div className="settings-modal" role="dialog" aria-modal="true" aria-label="项目设置">
        ...
        {isError && (
          <div className="settings-modal-body">
            ...
            <p className="muted small">无法读取设置，请确认 Studio Service 已启动。</p>
          </div>
        )}
        ...
        {!isLoading && !isError && form && (
          <div className="settings-modal-body">
              ...
              <div className="settings-row">
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    placeholder="模型名"
                    onChange={(e) => setField("default_image_model", e.target.value)}
                  />
                </label>
              </div>
              <label className="field">
                <span className="field-label">默认 Workflow</span>
                <input
                  value={form.default_image_workflow_id}
                  placeholder="如 default_image_api"
                  onChange={(e) => setField("default_image_workflow_id", e.target.value)}
                />
              </label>
            </section>
              ...
              <div className="settings-row">
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    placeholder="模型名"
                    onChange={(e) => setField("default_video_model", e.target.value)}
                  />
                </label>
              </div>
              <label className="field">
                <span className="field-label">默认 Workflow</span>
                <input
                  value={form.default_video_workflow_id}
                  placeholder="视频 Workflow"
                  onChange={(e) => setField("default_video_workflow_id", e.target.value)}
                />
              </label>
            </section>
              ...
              <div className="settings-row">
                <label className="field">
                  ...
                  <input
                    ...
                    placeholder="配音 Provider"
                    onChange={(e) => setField("default_voice_provider", e.target.value)}
                  />
                </label>
                <label className="field">
                  <span className="field-label">Model</span>
                  <input
                    value={form.default_voice_model}
                    placeholder="模型名"
                    onChange={(e) => setField("default_voice_model", e.target.value)}
                  />
                </label>
              </div>
            </section>
            ...
            <section className="settings-section">
              ...
              <div className="settings-row">
                <label className="field">
                  <span className="field-label">自动重试次数</span>
                  <input
                    type="number"
                    min={0}
                    value={form.auto_retry}
                    onChange={(e) => setField("auto_retry", Math.max(0, Number(e.target.value) || 0))}
                  />
                </label>
                <label className="field">
                  <span className="field-label">最大重试上限</span>
                  <input
                    type="number"
                    min={0}
                    value={form.max_retry_count}
                    onChange={(e) => setField("max_retry_count", Math.max(0, Number(e.target.value) || 0))}
                  />
                </label>
              </div>
              <ToggleRow
                ...
                onChange={(v) => setField("auto_save", v)}
              />
              <ToggleRow
                label="连贯性检查"
                hint="生成时执行连续性校验"
                checked={form.continuity_enabled}
                onChange={(v) => setField("continuity_enabled", v)}
              />
              <ToggleRow
                label="新版本自动启用"
                hint="新生成版本自动设为当前"
                checked={form.auto_activate_new_generation}
                onChange={(v) => setField("auto_activate_new_generation", v)}
              />
            </section>
            ...
            {saved && (
              <div className="settings-saved-note">
                <CheckCircle size={16} weight="fill" /> 设置已保存
              </div>
            )}
            ...
            <div className="settings-modal-actions">
              ...
              <button
                ...
                onClick={() => form && save.mutate(toUpdate(form))}
              >
                {save.isPending ? (
                  "保存中…"
                ) : (
