  ...
  return (
    <aside className="provenance-panel" aria-label="资产溯源">
      <header className="provenance-head">
        ...
        <button type="button" className="icon-button" aria-label="关闭溯源" onClick={onClose}>
          <X size={16} />
        </button>
      </header>
...
function ProvenanceContent({ data }: { data: ProvenanceRead }) {
  const hasProvenance =
    !!data.generation || data.inputs.length > 0 || data.retry_of || data.parent_asset_id || data.ancestors.length > 0;
  if (!hasProvenance) {
    return (
      <div className="provenance-body">
        <div className="provenance-empty">
          <ImageSquare size={30} />
          <p>这个资产没有溯源记录。</p>
          <span>它可能是直接导入或由系统生成的，未登记对应的生成链路。</span>
        </div>
      </div>
    );
  }
        ...
        <section className="provenance-section">
          ...
          <div className="provenance-chain">
            {data.ancestors.map((ancestorId) => (
              <ChainNode key={ancestorId} id={ancestorId} kind="ancestor" />
            ))}
          ...
          {data.parent_asset_id && (
            <p className="provenance-note">
              父资产 <code>{shortId(data.parent_asset_id)}</code>（当前资产由它派生）
            </p>
          )}
        </section>
      )}
        ...
        <section className="provenance-section">
          ...
          <div className="provenance-generation">
            <div className="provenance-meta">
              ...
              {data.generation.prompt_version_id && (
                <KV k="Prompt v" v={shortId(data.generation.prompt_version_id)} mono />
              )}
...
function ChainNode({ id, kind }: { id: string; kind: string }) {
  return (
    <span className={"provenance-chain-node " + kind}>
      <GitFork size={13} />
      {shortId(id)}
    </span>
  );
...
function InputRow({ input }: { input: GenerationInputRead }) {
  ...
  return (
    <div className="provenance-input-row">
      ...
      <div>
        <strong>{input.role ?? referenceType}</strong>
        <small>
          {referenceType}
          <span className="sep">·</span>
          {input.reference_id ? shortId(input.reference_id) : "—"}
        </small>
      </div>
