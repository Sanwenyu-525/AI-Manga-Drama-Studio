        ...
        {workflows && workflows.length > 0 && (
          <div className="workflow-layout">
            <div className="workflow-list">
              {workflows.map((w) => (
                <button key={w.id} type="button" className={`workflow-card ${selectedId === w.id ? "selected" : ""}`} onClick={() => setSelectedId(w.id)}>
                  <span className="workflow-card-head">
                    <GearSix size={17} />
                    <strong>{w.id}</strong>
                    {w.is_default && <span className="badge warn">默认</span>}
                  </span>
                  <small>{w.file}{w.exists ? "" : "（缺失）"}</small>
                  <span className="workflow-meta"><TreeStructure size={13} /> {w.node_count ?? "—"} 节点</span>
                </button>
              ))}
            </div>
              ...
              {selected ? (
                <>
                  ...
                  <div className="workflow-facts">
                    <div><span>模板文件</span><strong>{selected.file}</strong></div>
                    <div><span>节点数量</span><strong>{selected.node_count ?? "—"}</strong></div>
                    <div><span>输出节点</span><strong>{selected.output_node_class}</strong></div>
                    <div><span>状态</span><strong>{selected.valid_json === false ? "无效 JSON" : selected.exists ? "可用" : "文件缺失"}</strong></div>
                  </div>
                  <div className="workflow-section">
                    <span className="field-label">节点类型</span>
                    <div className="node-type-chips">
                      {(selected.node_types ?? []).map((t) => (<span key={t} className="node-chip">{t}</span>))}
                    </div>
                  </div>
                  <div className="workflow-section">
                    <span className="field-label">占位符令牌</span>
                    <div className="node-type-chips">
                      {(selected.placeholder_tokens ?? []).map((t) => (<code key={t} className="token-chip">{t}</code>))}
                    </div>
                    <p className="muted small">Studio 仅注入 $PROMPT / $SEED / $WIDTH / $HEIGHT 等参数，不感知 KSampler/CLIP 等节点语义（契约 §42）。</p>
                  </div>
                  <div className="workflow-section">
