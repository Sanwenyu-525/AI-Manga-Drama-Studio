...
export function StoryboardView({ sceneId }: { sceneId: string }) {
  ...
  const readyCount =
    storyboard?.shots.filter((shot) => shot.status === "image_ready" || shot.status === "approved").length ?? 0;
    ...
    <div className="storyboard">
      ...
      <div className="storyboard-toolbar">
        ...
        <div className="row gap">
          <button className="btn secondary" onClick={() => createShot.mutate()} disabled={createShot.isPending}>
            <Plus size={15} /> 镜头
          </button>
          <button
            className="btn primary"
            disabled={generateShots.isPending || Boolean(planOpId)}
