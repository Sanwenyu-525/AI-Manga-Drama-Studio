...
import { ArrowsClockwise, CaretDown, CaretUp, CheckCircle, ClockCounterClockwise, HourglassMedium, ListBullets, Pause, Play, WarningCircle, X } from "@phosphor-icons/react";
    ...
    <div className="queue-dock">
      <div className="queue-header">
        <div className="queue-tabs">
          <button className={tab === "queue" ? "active" : ""} onClick={() => setTab("queue")}><ListBullets size={16} /> 生成队列 <span>{Object.keys(live).length}</span></button>
        ...
        {tab === "jobs"
          ...
          : <GenerationSummary live={live} history={history} />}
        <button className="icon-button dock-toggle" onClick={() => setExpanded(!expanded)} aria-label={expanded ? "收起" : "展开"}>
      ...
      <div className="queue-body">
        {tab === "jobs"
          ...
          : (
            <>
              {tab === "queue" && Object.keys(live).map((key) => {
                ...
                return (
                  <div key={generation.id} className="queue-item running">
                    ...
                    <button className="icon-button" onClick={() => cancelGeneration.mutate(generation.id)} title="取消任务"><X size={15} /></button>
                  </div>
                );
              })}
              ...
              {(() => {
                ...
                return visible.slice(0, expanded ? 10 : 3).map((generation) => (
                  <div key={generation.id} className={`queue-item ${generation.status}`}>
                    ...
                    {generation.status === "failed" && <button className="btn secondary compact" onClick={() => retryGeneration.mutate(generation.id)} disabled={retryGeneration.isPending}><ArrowsClockwise size={14} /> 重试</button>}
                  </div>
                ));
              })()}
              ...
              {(() => {
                const runningIds = new Set(Object.values(live).map((item) => item.id));
                const persisted = (history ?? []).filter((item) => !runningIds.has(item.id));
                const hasQueue = tab === "queue" ? Object.values(live).length + persisted.filter((item) => item.status !== "completed").length : persisted.filter((item) => item.status === "completed").length;
                if (tab === "queue" && Object.values(live).length === 0 && hasQueue === 0) {
                  return <div className="queue-empty"><CheckCircle size={17} /> 当前没有待处理的生成任务</div>;
                }
                if (tab === "history" && persisted.length === 0) {
                  return <div className="queue-empty"><CheckCircle size={17} /> 还没有已完成记录</div>;
                }
                return null;
              })()}
            </>
...
function GenerationSummary({ live, history }: { live: Record<string, { id: string }>; history?: GenerationRead[] }) {
  ...
  return (
    <div className="queue-summary"><span>排队 {Object.values(live).length}</span>{failedCount > 0 && <span className="failed-summary">失败 {failedCount}</span>}</div>
  );
}
...
function JobsPanel({ projectId, jobs, expanded }: { projectId?: string; jobs?: JobSummaryRead[]; expanded: boolean }) {
  ...
  if ((jobs ?? []).length === 0) {
    return <div className="queue-empty"><CheckCircle size={17} /> 暂无生成任务</div>;
  }
  return (
    <div className="jobs-list">
      {list.map((job) => <JobRow key={job.id} projectId={projectId} job={job} />)}
    </div>
  );
}
...
function JobRow({ projectId, job }: { projectId?: string; job: JobSummaryRead }) {
  ...
  return (
    <div className={`job-row ${job.status} ${open ? "open" : ""}`}>
      <div className="job-head" onClick={() => setOpen((o) => !o)} role="button" tabIndex={0} aria-expanded={open}
        ...
        <div className="job-title">
          ...
          <span>{JOB_TYPE_TEXT[job.job_type] ?? job.job_type} · {job.task_count} 任务 · {formatTime(job.created_at)}</span>
        </div>
        {isInterrupted(job.status) && <span className="badge warn recovery-badge"><WarningCircle size={12} weight="fill" /> 已中断 · 可恢复</span>}
        <div className="progress-track job-progress"><div className="progress-fill" style={{ width: `${job.progress}%` }} /></div>
      ...
      <div className="job-detail">
        <div className="job-actions-row">
          <JobActions
            canPause={canPause} canResume={canResume} canCancel={canCancel} canRetry={canRetry}
            onPause={() => pause.mutate()} onResume={() => resume.mutate()} onCancel={() => cancel.mutate()} onRetry={() => retry.mutate()}
            pending={pause.isPending || resume.isPending || cancel.isPending || retry.isPending}
          />
          {isRecoverable(job.status) && <span className="recovery-hint"><WarningCircle size={14} /> 已中断/暂停 · 可恢复</span>}
        </div>
...
function JobActions({ canPause, canResume, canCancel, canRetry, onPause, onResume, onCancel, onRetry, pending }: {
  canPause: boolean; canResume: boolean; canCancel: boolean; canRetry: boolean;
  onPause: () => void; onResume: () => void; onCancel: () => void; onRetry: () => void; pending: boolean;
}) {
  return (
    <div className="job-actions">
      {canPause && <button className="btn secondary compact" onClick={onPause} disabled={pending}><Pause size={14} /> 暂停</button>}
      {canResume && <button className="btn primary compact" onClick={onResume} disabled={pending}><Play size={14} weight="fill" /> 恢复</button>}
      {canCancel && <button className="btn secondary compact" onClick={onCancel} disabled={pending}><X size={14} /> 取消</button>}
      {canRetry && <button className="btn secondary compact" onClick={onRetry} disabled={pending}><ArrowsClockwise size={14} /> 重试失败任务</button>}
    </div>
  );
}
      ...
      <tbody>
        {detail.tasks.map((task) => (
          <tr key={task.id} className={task.status}>
            ...
            <td><span className={`badge ${task.status}`}>{generationStatusText(task.status)}</span></td>
            <td className="task-progress"><span className="progress-track"><span className="progress-fill" style={{ width: `${task.progress}%` }} /></span>{task.progress}%</td>
            <td className="mono muted">{task.generation_id ? task.generation_id.slice(-8) : "—"}</td>
            <td className="job-error" title={task.error_message ?? undefined}>{task.error_message || "—"}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
function generationStatusText(status: string): string {
  return ({ failed: "失败", completed: "完成", cancelled: "已取消", queued: "排队中", running: "生成中", retrying: "重试中", paused: "已暂停", interrupted: "已中断", skipped: "已跳过", dependency_failed: "依赖失败" } as Record<string, string>)[status] ?? status;
}
