...
- ✅ **真实 LLM / 真实 ComfyUI（2026-08）**：`STUDIO_LLM_MODE=openai` 指向 Agnes `agnes-2.5-flash`；`STUDIO_IMAGE_PROVIDER=agnes`；本地 ComfyUI 0.33 + **Z-Image-Turbo int8** 模板已注册（`workflows/zimage_turbo_api.json` + `WORKFLOW_CATALOG["zimage_turbo"]`，UNETLoader + TextEncodeZImageOmni + qwen_3_4b CLIP，8 步 euler/cfg 2.5）
