...
- ⬜ **下一步 = Stage D（Agent）**：LangGraph Director Graph（Understand→Context→Plan→Execute→Review）+ 三个工具（get_shot / update_shot / generate_image）+ `POST /api/v1/agent/director/runs`（202）+ agent 事件流 + 前端 AI Director 面板（Selection-aware，Scenario A/B/C 验收）
