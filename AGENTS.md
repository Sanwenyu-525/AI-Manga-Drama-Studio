...
- ✅ **自主迭代 06 — 场景信息编辑：AI 拆解后的场景环境可人工细化（2026-09-01，Post-MVP）**：`scene.time_of_day/lighting/weather/mood/description` 只读展示（场景头 meta + ShotInspector sceneOk），无编辑入口；而 `PATCH /scenes/{id}` 早已支持全部字段、且 `update_scene` 内置 **P8-T017 hook**（场景基准变更 → 连续性重算 + 活跃资产 stale 标记）→ 后端 100% 就绪、只缺前端。**本轮（纯前端，零后端改动）**：新增 `ScenePropertiesEditor`（时段/光照/天气/氛围/描述，脏检查 + PATCH {revision, patch} + 409 冲突提示 + ApiErrorPanel 复用）+ StoryboardView 场景头「编辑场景」按钮/内联面板（与迭代 03 地点绑定同区同模式）；保存后 invalidate scene/storyboard/scenes/sceneContinuity/continuityWarnings。验证：pytest **633 passed +1 skip**（本轮零后端改动；test_continuity+test_episode_scene_crud 29 项通过）、vitest **298 passed**（新增 4：预填/脏检查+PATCH 负载/409 冲突/保存成功 onClose）、tsc/eslint/ruff/build 全绿 + live smoke `scripts/smoke_scene_edit.py` 5 步全通（PATCH 时段/光照/氛围 → revision 1→2 → P8-T017 连续性重算把环境基准传播到 scene_continuity_states）。报告 `docs/reports/autonomous-iteration-2026-09-01-scene-properties-edit.md`
- ✅ **自主迭代 07 — AI Director update_scene 工具：场景级 Agent 能力（2026-09-01，Post-MVP）**：Agent 只能改镜头（update_shot），用户说「把这场戏改成夜晚」无法达成——工具面缺场景级修改。**后端**：`UpdateSceneArgs` + `TOOL_SCHEMAS` 注册 + `_update_scene` 处理器（**R1 自动应用**走 SceneService（触发 P8-T017 连续性重算 + stale）+ **scene ChangeSet 可撤销** + resume 幂等；`ToolOperation.tool` Literal 增 `update_scene`；risk.py `update_scene`→R1；**ChangeSetService `record_scene_patch` + `_undo_scene_patch` + undo 分发**（entity_type="scene"）；fake_planner 场景意图识别（目标词「场景/这场戏/这场」+ 环境值词 → update_scene；无值/无选中场景 → 澄清）+ fake.py `scene_id_from_prompt`；graph execute_node 放宽 shot 强制（场景工具免 resolved_shot_id）。**前端（标签层）**：`changeSetToolLabel`（修改场景）+ `SHOT_FIELD_LABELS` 补场景字段（时段/光照/天气/氛围/描述）。验证：pytest **638 passed +1 skip**（新增 5：应用+ChangeSet/撤销恢复/R1 无审批/P8-T017 传播/planner 意图边界；既有 agent 全套 51 项通过）、vitest **298 passed**、tsc/eslint/ruff/build 全绿 + live smoke `scripts/smoke_agent_scene.py` 7 步全通（白天→「改成夜晚」→应用→scene ChangeSet→P8-T017 传播→撤销恢复白天）。报告 `docs/reports/autonomous-iteration-2026-09-01-agent-update-scene.md`
- ✅ **自主迭代 08 — 过期镜头闭环：连续性 stale 可见 + 批量重生成（2026-09-01，Post-MVP）**：迭代 06/07 的场景编辑与 Agent update_scene 都会把镜头活跃图片资产标 stale，但前端**不展示也不提供批量重生成**——「编辑→过期→重生成」循环断点。**后端**：`ShotSummary.image_stale`（get_storyboard 一次批量查询活跃图片资产 status="stale"，零 Schema）+ 契约 §100 DTO 注释。**前端**：ShotCard「过期」徽标（.badge.stale 红）+ 生成菜单「重新生成过期镜头」（staleShots = image_stale && 无进行中，走 batchSubmit allSettled 聚合）。验证：pytest **641 passed +1 skip**（新增 3：生成后 image_stale=false / 场景编辑→true（含资产层确证）/ 重生成→false）、vitest **299 passed**（新增 1：过期徽标）、tsc/eslint/ruff/build 全绿 + live smoke `scripts/smoke_stale_loop.py` 5 步全通（生成→场景编辑 stale→重生成清除）。报告 `docs/reports/autonomous-iteration-2026-09-01-stale-shot-loop.md`
- ✅ **自主迭代 09 — 契约防回归 CI：前端调用点↔OpenAPI 全量比对（2026-09-01，Post-MVP）**：全栈联调曾一次抓到 4 P0 + 6 P1 契约断点，但一直无自动防线（此前建议 #1 反复被推迟）。本轮落地**零新 CI 基建**方案：`backend/scripts/check_frontend_contract.py`（扫描 `frontend/src/**/*.{ts,tsx}` 全部 `api.<method>(path)` 调用点 → 与 FastAPI `app.openapi()` **同进程实时生成**的 spec 逐段比对，零快照过期风险）；**segment-wise 通配匹配**统一处理模板字符串 `${expr}`→`*`、`"a"+x+"b"` 拼接、三元段（`${cond?"approve":"reject"}`→`*`）、query 丢弃、以及动态前缀（EntityVersionBlock `${base}${entityId}/versions`→`* */versions`）；`api.upload`→POST、`{param}`→`*`。**CI 门**：`backend/tests/test_frontend_contract.py`（8 项：全量比对 + spec 规模守卫 + 6 个归一化单元用例）随既有 pytest 跑，前端调用指向不存在的路由/错误 method 即失败并给出 `file:line`（反回归实证：删 `/providers` + `/generations/recent` 两条路由 → 精确报出 4 处断点）。当前基线 **175 调用点 ↔ 175 operations 全匹配**。验证：pytest **649 passed +1 skip**（新增 8）、vitest 299、tsc/eslint/ruff/build 全绿。报告 `docs/reports/autonomous-iteration-2026-09-01-contract-anti-regression.md`
